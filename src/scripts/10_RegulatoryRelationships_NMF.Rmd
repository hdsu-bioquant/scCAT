---
title: "scCAT-seq Non-Negative Matrix Factorization Transcriptome (RNAseq)"
...
#setting working directory, Change to match user file system!
#Should be home directory of the project
knitr::opts_knit$set(root.dir=params$path_wd)
...
## Data preprocessing and applying NMF

Applying Non-Negative Matrix Factorization (NMF) to Regulatory Relationships matrix

Dimension of the *matrix*:
  
```{r dataloading, results='asis', eval=TRUE}
##––––––––––––––––––––––––––––––––––––––––––––––––––––––––––––––––––––––––––––##
##                            read matrix                                     ##
##––––––––––––––––––––––––––––––––––––––––––––––––––––––––––––––––––––––––––––##

norm.mat <- readRDS(params$regrel)

tmp.dim <- data.frame(dim(norm.mat))
colnames(tmp.dim) <- "Value"
rownames(tmp.dim) <- c("Regulatory Relationships","Cells")
#matrix dimension
kable(tmp.dim) 
# kable(tmp.dim) %>%
...
path.NMF.run <- paste0(params$work_dir, "/", params$assayID, "/rna_atac/regulatoryRelationships_", (params$window/1000),  "Kb/NMF/", params$assayID, "_NMF_regulatoryRelationships.RDS")
```
...
if (file.exists(path.NMF.run)) {
  ...
  # save results
  saveRDS(nmf.exp, file = params$nmf)
}
...
figs.path <- paste0(params$work_dir, "/", params$assayID, "/figs/regulatoryRelationships_", (params$window/1000), "Kb")

#create directory to save plots
dir.create(figs.path, recursive = TRUE)

#save K stats
pdf(file = paste0(figs.path, "/regulatoryRelationships_NMF_factorization.metrics.pdf"), width=8, height=6)
gg.optKr
dev.off()
...
pdf(file = paste0(figs.path, "/regulatoryRelationships_NMF_riverplot.pdf"), width=8, height=6)
plot(river, plot_area=1, yscale=0.6, nodewidth=0.5)
dev.off()
...
pdf(file = paste0(figs.path, "/regulatoryRelationships_NMF_Hmatrix_wnorm_heatmap.pdf"), width=10, height=7)
...
#save H matrix heatmap
pdf(file = paste0(figs.path, "/regulatoryRelationships_NMF_Hmatrix_hnorm_heatmap.pdf"), width=10, height=7)
for(ki in names(h.heatmap.list)) {
  print(h.heatmap.list[[as.character(ki)]])
}
dev.off()
...
path_results_NMF <- paste0(params$assayID, "/rna_atac/regulatoryRelationships_", (params$window/1000),  "Kb/NMF/Wnormalized")
...
path_results_NMF <- paste0(params$assayID, "/rna_atac/regulatoryRelationships_", (params$window/1000),  "Kb/NMF/Hnormalized")
