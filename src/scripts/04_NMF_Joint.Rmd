---
title: "scCAT-seq Non-Negative Matrix Factorization Joint (RNAseq & ATACseq)"
...
Read normalized gene expression and peak accessibility matrices
...
cage.norm.mat <- list(rna  = readRDS(params$matrix_rna),
                      atac = readRDS(params$matrix_atac))
...
##––––––––––––––––––––––––––––––––––––––––––––––––––––––––––––––––––––––––––––##
tmp.dim <- data.frame(dim(cage.norm.mat$rna))
...
tmp.dim <- data.frame(dim(cage.norm.mat$atac))
colnames(tmp.dim) <- "Value"
...
# Data Concatenation
The matrix of normalized gene expression is concatenated with the normalized peaks accessibility matrix
  
```{r concatmat, results="asis", cache=FALSE, message=FALSE}
# concatenate normalized matrices
norm.mat <- do.call(rbind, cage.norm.mat)

tmp.dim <- data.frame(dim(norm.mat))
colnames(tmp.dim) <- "Value"

cat("Dimension of transcriptome (RNAseq) +   \n  ") 
cat("chromatin accessibility (ATACseq) dataset  \n  ") 
kable(tmp.dim) 
...
path.NMF.run <- paste0(params$assayID, "/", params$omicsID, "/NMF/", params$assayID, "_NMF_", params$omicsID, ".RDS")
```
