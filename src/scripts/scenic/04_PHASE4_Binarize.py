...
if __name__ == '__main__':
    DATA1_FOLDER="/icgc/dkfzlsdf/analysis/B080/crg/keding/scGRN/pyScenicGlioblastoma"
    DATA2_FOLDER="/icgc/dkfzlsdf/analysis/B080/quintera/Projects/scCATseq_ATAC_RNA/results_human_embryo/rna/scater"
    TMP_FOLDER="/icgc/dkfzlsdf/analysis/B080/quintera/Projects/scCATseq_ATAC_RNA/results_human_embryo/rna/scenic/phases"
    OUT_FOLDER="/icgc/dkfzlsdf/analysis/B080/quintera/Projects/scCATseq_ATAC_RNA/results_human_embryo/rna/scenic"
    
    FEATHER_GLOB            = os.path.join(DATA1_FOLDER, "HumanRankingDatabases/hg*.feather")
    MOTIF_ANNOTATIONS_FNAME = os.path.join(DATA1_FOLDER, "HumanMotifAnnotation/motifs-v9-nr.hgnc-m0.001-o0.0.tbl")
    HG_TFS_FNAME            = os.path.join(DATA1_FOLDER, "HumanMotifAnnotation/hg_tfs.txt")
    #path to the expression matrix
    SC_EXP_FNAME            = os.path.join(DATA2_FOLDER,"rna.norm.sce.mat.csv")
    ...
    REGULONS_FNAME          = os.path.join(OUT_FOLDER,  "regulons.txt")
    AUC_FNAME               = os.path.join(OUT_FOLDER,  "AUC.csv")
    BINARYAUC_FNAME               = os.path.join(OUT_FOLDER,  "binary_AUC.csv")
    BINARYTHR_FNAME               = os.path.join(OUT_FOLDER,  "binary_thresholds.csv")
    CLUSTERMAP_FNAME        = os.path.join(OUT_FOLDER,  "Clustermap.png")
    NOMENCLATURE            = "HGNC"
    ...
    #auc_thresholds_txt = open(BINARYTHR_FNAME, "w")
    #auc_thresholds_txt.write(auc_thresholds)
    #auc_thresholds_txt.close()
